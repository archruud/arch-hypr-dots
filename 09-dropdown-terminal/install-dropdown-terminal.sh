...
# ═════════════════════════════════════════════════════════════
# DROPDOWN TERMINAL - INSTALLSCRIPT
# Setter opp dropdown terminal for Hyprland med JaKooLit script
# ═════════════════════════════════════════════════════════════

echo "╔══════════════════════════════════════════════════════════════╗"
echo "║          DROPDOWN TERMINAL SETUP FOR HYPRLAND               ║"
echo "║          (Basert på JaKooLit sitt Dropterminal.sh)          ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
...
# Sjekk jq (KRITISK - scriptet trenger dette!)
if ! command -v jq &> /dev/null; then
    echo "  • jq - Ikke installert (NØDVENDIG for scriptet!)"
    PACKAGES_NEEDED+=("jq")
else
    echo "  ✅ jq - Allerede installert"
fi

# Sjekk kitty (standard terminal)
if ! command -v kitty &> /dev/null; then
    echo "  • kitty - Ikke installert"
    PACKAGES_NEEDED+=("kitty")
else
    echo "  ✅ kitty - Allerede installert"
fi
...
# Lag scripts mappe hvis den ikke finnes
echo "📁 Lager ~/.config/hypr/scripts/ mappe..."
mkdir -p ~/.config/hypr/scripts

# Kopier Dropterminal.sh script
echo "📝 Lager Dropterminal.sh script..."
cat > ~/.config/hypr/scripts/Dropterminal.sh << 'SCRIPT_END'
#!/bin/bash
...
chmod +x ~/.config/hypr/scripts/Dropterminal.sh
echo "✅ Dropterminal.sh opprettet og gjort kjørbart!"

# Backup hyprland.conf
echo ""
echo "📦 Backup av hyprland.conf..."
if [ -f ~/.config/hypr/hyprland.conf ]; then
    cp ~/.config/hypr/hyprland.conf ~/.config/hypr/hyprland.conf.backup-$(date +%Y%m%d-%H%M%S)
    echo "✅ Backup lagret"
fi

# Sjekk om keybind allerede finnes
if grep -q "Dropterminal.sh" ~/.config/hypr/hyprland.conf 2>/dev/null; then
    echo "⚠️  Dropdown keybind finnes allerede i hyprland.conf"
else
    echo ""
    echo "📝 Legger til keybind i hyprland.conf..."
    
    cat >> ~/.config/hypr/hyprland.conf << 'CONFIG_END'

# ═════════════════════════════════════════════════════════════
# DROPDOWN TERMINAL (JaKooLit Dropterminal.sh)
# INGEN window rules nødvendig - alt håndteres i scriptet!
# ═════════════════════════════════════════════════════════════

# Keybind: SUPER + SHIFT + RETURN for dropdown terminal
bind = SUPER SHIFT, RETURN, exec, ~/.config/hypr/scripts/Dropterminal.sh kitty

CONFIG_END
    
    echo "✅ Keybind lagt til!"
fi

# Last inn ny konfigurasjon
echo ""
echo "🔄 Laster inn Hyprland konfigurasjon..."
hyprctl reload
...
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
echo "✅ Dropdown terminal er nå satt opp!"
echo ""
echo "📋 Bruk:"
echo "   • SUPER + RETURN         = Vanlig terminal"
echo "   • SUPER + SHIFT + RETURN = Dropdown terminal (toggle)"
echo ""
echo "🎨 Animasjoner:"
echo "   • Slide down når du åpner"
echo "   • Slide up når du lukker"
echo "   • Smooth og profesjonell!"
echo ""
echo "⚙️  Endre størrelse (i Dropterminal.sh):"
echo "   WIDTH_PERCENT=50   # Bredde (standard: 50%)"
echo "   HEIGHT_PERCENT=50  # Høyde (standard: 50%)"
echo "   X_PERCENT=25       # X posisjon (standard: 25% = sentrert)"
echo "   Y_PERCENT=5        # Y posisjon (standard: 5% fra topp)"
echo ""
echo "🔧 Endre terminal:"
echo "   Rediger keybind i hyprland.conf:"
echo "   bind = SUPER SHIFT, RETURN, exec, ~/.config/hypr/scripts/Dropterminal.sh foot"
echo "   (Støtter: kitty, foot, alacritty, ghostty)"
echo ""
echo "📝 Filer:"
echo "   • Script: ~/.config/hypr/scripts/Dropterminal.sh"
echo "   • Keybind: ~/.config/hypr/hyprland.conf (nederst)"
echo "   • Address cache: /tmp/dropdown_terminal_addr"
echo "   • Backup: ~/.config/hypr/hyprland.conf.backup-[timestamp]"
echo ""
echo "💡 Tips:"
echo "   • INGEN window rules nødvendig - alt håndteres i scriptet!"
echo "   • Debug mode: ~/.config/hypr/scripts/Dropterminal.sh -d kitty"
echo "   • Terminalen følger deg til alle workspaces (pinned)"
echo ""
