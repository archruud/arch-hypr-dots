...
LOGFILE="$HOME/network_script-$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "$LOGFILE") 2>&1
echo "════════════════════════════════════════════════════"
echo " network_script — standalone"
echo " Startet: $(date)"
echo " Logg: $LOGFILE"
echo "════════════════════════════════════════════════════"
echo ""
...
if [ -f "$HYPR_CONF" ] && ! grep -q "# Network keybinds" "$HYPR_CONF"; then
    log "Legger til network keybinds i hyprland.conf..."
    cat << 'KEYBINDS' >> "$HYPR_CONF"

# Network keybinds
bind = SUPER, N,       exec, dolphin smb:/
bind = SUPER SHIFT, N, exec, dolphin remote:/
bind = SUPER CTRL, N,  exec, dolphin mtp:/
bind = SUPER ALT, N,   exec, ~/.local/bin/scan-network
KEYBINDS
    success "Network keybinds lagt til"
elif [ -f "$HYPR_CONF" ]; then
    success "Network keybinds finnes allerede"
else
    warning "Hyprland config ikke funnet"
fi
